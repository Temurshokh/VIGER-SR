"""VIGER Telegram bot: Python-only image SR + a tiny self-trained language model."""
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    context.user_data.setdefault("voice", False)
    await update.message.reply_text(
        "⚡ VIGER AI lab\n\n"
        "📸 Photo → choose AI 2×/4× → get an enhanced image.\n"
        "💬 Text → my own tiny neural LM generates the answer.\n"
        "🧪 /teach Hi => Hello! adds a new training example.\n"
        "🧠 /train retrains the LM from scratch on the current corpus.\n"
        "🔊 /voice on|off enables voice replies.\n\n"
        "Python only. No Visual Studio, no .exe, no local model server.",
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    await update.message.reply_text(
        ...
        "💬 Text: generated by the self-trained TinyLM.\n"
        "🧪 /teach question => answer\n"
        "🧠 /train retrains from the corpus.\n"
        "🔊 /voice on|off\n"
        "ℹ️ /status"
    )
...
async def status_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    try:
        ...
        device = "unknown"
    await update.message.reply_text(
        f"🧠 TinyLM: {'loaded' if _lm is not None else 'not loaded'}\n"
        f"🖼️ SR: {'loaded' if _sr is not None else 'not loaded'}\n"
        f"⚙️ Device: {device}\n"
        "📁 LM: artifacts/viger_tiny_lm.pt"
    )
...
async def train_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    status = await update.message.reply_text("🧠 Training TinyLM from scratch…")
    try:
        from python.viger_tiny_lm import train
        started = time.perf_counter()
        await asyncio.to_thread(train, CORPUS, CHECKPOINT, 700)
        await asyncio.to_thread(reload_lm)
        elapsed = time.perf_counter() - started
        await status.edit_text(f"✅ TinyLM trained from scratch in {elapsed:.1f}s")
    except Exception as exc:
        await status.edit_text(f"❌ TinyLM training failed.\n\n{type(exc).__name__}: {exc}")
...
async def teach_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text(
        ...
        "Run /train to teach the neural model on the updated data."
    )
...
async def process_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        await status.edit_text(
            "🧠 TinyLM could not generate a response.\n\n"
            f"{type(exc).__name__}: {exc}"
        )
