...
async def status_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if _lm is not None:
        ...
        lm_text = f"TinyGPT v4 • {params:,} params • vocab {tokenizer.vocab_size}"
...
async def train_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    raw_steps = context.args[0] if context.args else "3500"
    try:
        steps = max(100, min(int(raw_steps), 10000))
    except ValueError:
        await update.message.reply_text("Use /train or /train 3500")
        return

    context.user_data["learn"] = False
    status = await update.message.reply_text(
        f"🧠 Training TinyGPT v4 from scratch…\nSteps: {steps}\nThis can take a while on CPU."
    )
    try:
        ...
        await status.edit_text(f"✅ TinyGPT v4 retrained from scratch in {elapsed:.1f}s")
